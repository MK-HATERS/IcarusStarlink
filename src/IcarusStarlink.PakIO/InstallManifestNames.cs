...
/// services that write it (RebuildService, InstallService) and the one that reads it back for the
/// installed-vs-list comparison (6.6), so a rename can't silently drift out of sync between a
/// writer and a reader. UE4SS mods have no equivalent manifest (Phase 8.5) — a mod's real presence
/// in the game's own Mods folder IS its state, nothing else to track.
/// </summary>
