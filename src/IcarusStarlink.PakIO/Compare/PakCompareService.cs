...
public sealed class PakCompareService(IUnrealPakService unrealPakService) : IPakCompareService
{
    ...
    private static PakCompareResult Compare(string firstDirectory, string secondDirectory)
    {
        ...
        foreach (var relativePath in allPaths)
        {
            ...
            if (inFirst && inSecond)
            {
                ...
                else if (!FilesAreIdentical(firstPath!, secondPath!))
                {
    ...
    private static bool FilesAreIdentical(string firstPath, string secondPath)
    {
        ...
        // Streamed rather than ReadAllBytes — a single asset inside a real merged pak can be a
        // multi-hundred-MB .ubulk texture, which two whole-file byte arrays would double up in
        // memory at once.
